...
export default function CreateExperience ({ list, action, update }) {
    ...
    const handleCreated = async () => {
        try {
            setLoading(true);
            const data = await createExperience(list, formData)
            update(prev => [data, ...prev]);
            toast.success('Éxito', { description: `Se creó con éxito la experiencia` })
        ...
        <>
            ...
            <div className="flex flex-col gap-2">
                ...
                <div className="w-full flex gap-2">
                    ...
                    <button className="w-full h bg-primary text-white rounded-md" style={{"--h":"40px"}} onClick={handleCreated}>{loading ? 'Creando...' : 'Crear'}</button>
                </div>
            </div>
        </>
    )
